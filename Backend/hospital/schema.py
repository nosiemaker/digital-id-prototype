...
class MedicalCertificateCauseOfDeathCreate(BaseModel):
    ...
    # --- Reference (auto-generated by the service) ---
...
class NoticeOfDeathCreate(BaseModel):
    ...
    # --- Reference ---
    # serial_number and application_no are auto-generated by the service layer
    # using UUID-based unique identifiers. No longer required in the request body.

   
    # --- Section A: Details of Deceased ---
    # Auto-populated from the linked MCCD instance:
    #   date_of_death        → mccd.death_date
    #   place_of_death       → "HEALTH_FACILITY"
    #   place_of_death_name  → health_worker.facility_name
    #   age_at_death         → parsed from mccd.age_stated
    #
    # Auto-populated from Citizen when deceased_din is resolved:
    #   surname, other_names, occupation, residential_address,
    #   date_of_birth, sex, nationality, national_identity_no,
    #   social_security_no, education_level
    #
    ...
    residential_address: Optional[str] = Field(None)
    district: str = Field(..., max_length=100)
    # place_of_death_other is auto-set to None by the service.
    date_of_birth: Optional[date] = Field(None)
    sex: Optional[Sex] = Field(None)
    nationality: Optional[str] = Field(None, max_length=100)
    national_identity_no: Optional[str] = Field(None, max_length=30)
    social_security_no: Optional[str] = Field(None, max_length=30)
    education_level: Optional[EducationLevel] = Field(None)
    informant_relationship: str = Field(..., max_length=100)

    # --- Section B: Cause of Death (FOR OFFICIAL USE ONLY) ---
    # All fields auto-populated from the linked MCCD at submission time.
    # DO NOT include in the request body. Source mapping:
    #   cause_a / cause_a_icd_code  → immediate_cause / immediate_cause_icd
    #   cause_b / cause_b_icd_code  → antecedent_cause / antecedent_cause_icd
    #   cause_c / cause_c_icd_code  → underlying_cause / underlying_cause_icd

    # --- Section C: Police / Brought-in-Dead Certificate ---
    # All fields auto-populated by the service at submission time.
    # DO NOT include in the request body.
    #
    # Auto-populated from Section A / MCCD:
    #   deceased_surname_police     → surname
    #   deceased_other_names_police → other_names
    #   deceased_age_police         → age_at_death
    #   passed_away_date            → date_of_death
    #   passed_away_time            → mccd.death_time
    #   passed_away_place           → place_of_death_name
    #   suddenly_suffering_from     → mccd.cause_a
    #   treatment_was_at            → place_of_death_name
    #
    # Auto-set to None (captured at a later stage):
    #   police_certifier_name, police_certifier_residence, police_certifier_relationship,
    #   is_natural_death, is_sudden_death_postmortem_required, police_no_and_rank,
    #   police_formation, police_officer_name, police_officer_signed, police_officer_date,
    #   doctors_remarks, pupils_dilated_and_fixed, certifying_doctor_name,
    #   certifying_doctor_signature, certifying_doctor_date

    # --- Section D: Details of Informant ---
    # All fields auto-populated from the authenticated Citizen instance at submission time.
    # DO NOT include in the request body. Source mapping:
    #   informant_surname             → citizen.full_name (last token)
    #   informant_other_names         → citizen.full_name (preceding tokens)
    #   informant_contact_no          → citizen.phone
    #   informant_national_id         → citizen.nrc
    #   informant_nationality         → citizen.nationality
    #   informant_residential_address → citizen.residential_address
    #   informant_postal_address      → citizen.postal_address or residential_address
    #   date_of_registration          → today's date
    # --- Section E: Appendices Checklist ---
    # All fields auto-populated by the service at submission time:
    #   has_mccd                  → True
    #   has_informant_national_id → bool(informant.nrc)
    #   has_coroner_report        → bool(police_fields) or death_type in (SUDDEN, UNNATURAL)

    # --- Informant's Declaration ---
    # All fields auto-populated by the service at submission time:
    #   informant_declaration_name      → informant.full_name
    #   informant_declaration_signature → None
    #   informant_declaration_date      → today's date
